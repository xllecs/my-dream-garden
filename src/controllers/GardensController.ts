...
import { notifyGardenHumidityChange } from "../IrrigationWebSocket.js"
...
export async function viewGarden(request: Request, response: Response) {
  ...
  redisClient.set(gardenCacheKey, JSON.stringify(garden), { EX: 600 })
...
export async function listGardens(request: Request, response: Response) {
  ...
  if (gardens.length > 0) {
    await redisClient.set(cacheKey, JSON.stringify(gardens), { EX: 600 })
    return response.json({ gardens })
  }

  return response.status(404).json({ message: "user doesn't have any gardens" })
}
...
export async function createGarden(request: Request, response: Response) {
  ...
  redisClient.set(`garden:${garden.gardenId}`, JSON.stringify(garden), { EX: 600 })
...
export async function updateGarden(request: Request, response: Response) {
  ...
  if (garden.userId !== userId) return response.status(403).json({ error: "you cannot update this garden as it does not belong to you" })
...
export async function deleteGarden(request: Request, response: Response) {
  ...
  if (garden.userId !== userId) return response.status(403).json({ error: "you cannot delete this garden as it does not belong to you" })
